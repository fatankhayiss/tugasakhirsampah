...
$query = "SELECT p.id_pengguna, p.nama_lengkap, p.username, p.no_telepon, p.alamat, p.email, p.status
          FROM pengguna p
...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-3xl mx-auto bg-white rounded-lg shadow-xl overflow-hidden">
        <div class="bg-gray-50 px-6 py-4 border-b border-gray-200 flex justify-between items-center">
            <h2 class="text-xl font-semibold text-gray-800">Edit Data Picker</h2>
        ...
        <form action="<?php echo BASE_URL; ?>index.php?page=picker/proses_simpan" method="POST" class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" id="username" name="username" required 
                           value="<?php echo htmlspecialchars($driver['username']); ?>"
                ...
                <div>
                    ...
                    <select id="status" name="status" required class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-sky-500 focus:border-sky-500">
                        <option value="aktif" <?php echo (($driver['status'] ?? 'aktif') === 'aktif') ? 'selected' : ''; ?>>Aktif</option>
                        <option value="nonaktif" <?php echo (($driver['status'] ?? 'aktif') === 'nonaktif') ? 'selected' : ''; ?>>Nonaktif</option>
                    </select>
                </div>
                <div>
            ...
            <div class="mt-8 pt-5 border-t border-gray-200 flex justify-end gap-3">
                ...
                <button type="submit" name="update_picker" 
