...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    try {
        ...
        // Cek apakah ada baris yang terhapus (memastikan yang dihapus adalah driver/picker dan ada di database)
        if (mysqli_stmt_affected_rows($stmt_pengguna) > 0) {
            mysqli_commit($koneksi);
            $_SESSION['success_message'] = "Data Picker berhasil dihapus.";
        } else {
            mysqli_rollback($koneksi);
            $_SESSION['error_message'] = "Data Picker tidak ditemukan atau tidak bisa dihapus.";
        }
        ...
        error_log("Error delete picker (ID: $id_pengguna): " . $e->getMessage());
    }
